...
def compute_posx_posy(pos, axial_width=None):
    if axial_width is None:
        axial_width = int(math.sqrt(pos.shape[0]))
        assert axial_width ** 2 == pos.shape[0], "If no width is supplied, assume grid is square. However, the number of tokens is not a perfect square."
    posx = (pos % axial_width).long()
    posy = torch.div(pos, axial_width, rounding_mode='floor').long()
    return posx, posy
